...
export const SearchPage = () => {
    ...
    const blockByHeight = useBlock(
        { height: { equalTo: parseInt(query) } },
        { skip: !maybeHeight }
    );

    const extrinsicsByName = useExtrinsicsByName(query, "BLOCK_HEIGHT_DESC", {
        skip: !maybeName,
    });
    const eventsByName = useEventsByName(query, "BLOCK_HEIGHT_DESC", {
        skip: !maybeName,
    });
    ...
    const showLoading =
        forceLoading || (allResources.some((it) => it.loading) && !showResults);
    const showNotFound = allResources.every((it) => it.notFound);
    const error = allResources.find((it) => it.error)?.error;
    const showError =
        error && allResources.every((it) => it.error || it.notFound);
    ...
    if (!forceLoading) {
        if (extrinsicByHash.data) {
            return (
                <Navigate
                    to={`/extrinsic/${extrinsicByHash.data.id}`}
                    replace
                />
            );
        }
        ...
        if (blockByHash.data || blockByHeight.data) {
            return (
                <Navigate
                    to={`/block/${
                        blockByHash.data?.id || blockByHeight.data?.id
                    }`}
                    replace
                />
            );
        ...
        return (
            <Card>
                <NotFound>
                    ...
                    <span css={queryStyle}>{query}</span>
                </NotFound>
            </Card>
        );
    }
        ...
        return (
            <Card>
                <ErrorMessage
                    message={
                        <>
                            Unexpected error occured while searching for{" "}
                            <span css={queryStyle}>{query}</span>
                        </>
                    }
