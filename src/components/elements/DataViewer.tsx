...
import { DecodedArg, RuntimeSpec } from "../../model";
...
type DataViewerMode = "json" | "parsed";

const MODES: (DataViewerMode | false)[] = ["parsed", "json"];
...
export function DataViewer(props: DataViewerProps) {
    ...
    if (metadata && !runtimeSpec) {
        console.warn(
            "If <DataViewer> is used with metadata argument, runtimeSpec argument should be passed too."
        );
    }

    const [mode, setMode] = useState<DataViewerMode>(
        defaultMode || (MODES.find(Boolean) as DataViewerMode)
    );
    ...
    return (
        <div
            css={[
                dataViewerStyle,
                simple ? simpleDataViewerStyle : embeddedDataViewerStyle,
            ]}
        >
