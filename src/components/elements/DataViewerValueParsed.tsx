...
const CallIndexValue = (props: CallIndexValueProps) => {
    ...
    const callName =
        runtimeSpec?.metadata.pallets[palletIndex]?.calls[callIndex]?.name ||
        "";
    const formattedCallName =
        callName.slice(0, 1).toLowerCase() + callName.slice(1);
    return (
        <div
            css={valueStyle}
        >{`${formattedModuleName}.${formattedCallName}`}</div>
    );
};
                    ...
                    {value.map((item, index) => (
                        <TableRow key={index}>
                            <TableCell css={arrayIndexCellStyle}>
                                <div css={arrayIndexStyle}>
                                    {itemsMetadata[index]?.name}
                                </div>
                            </TableCell>
                            <TableCell>
                                <DataViewerValueParsed
