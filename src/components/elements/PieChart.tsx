...
export type PieChartOptions = echarts.ComposeOption<
    | PieSeriesOption
    | TitleComponentOption
    | LegendComponentOption
    | TooltipComponentOption
    ...
    const chartOptions = useMemo(() => {
        ...
        for (const key in options) {
            if (Array.isArray(options[key])) {
                chartOptions[key] = (options[key]! as any[]).map((it) =>
                    deepmerge(pieChartDefaultOptions[key] || {}, it)
                );
            } else {
                chartOptions[key] = deepmerge(
                    pieChartDefaultOptions[key] || {},
                    options[key]!
                );
            }
        }
        ...
        if (containerRef.current) {
            ...
            echartRef.current.on("finished", () => {
                window.dispatchEvent(
                    new CustomEvent("chart-finished", {
                        detail: {
