...
import { SortDirection, SortOrder } from "../../model";
        ...
        <div {...restProps} data-class="table">
            {getExportCSV && (
                <div css={csvDownload}>
                    ...
                    >
                        {isDownloading && (
                            <img src={LoadingSpinner} css={spinnerStyle} />
                        )}
                        <span
                            style={{ marginLeft: isDownloading ? "24px" : 0 }}
                        >
                            Download CSV
                        </span>
                    </Button>
            ...
            <div css={tableOptions}>
                <div css={tableFiltering}>
                    ...
                    {filterMappings &&
                        filter &&
                        Object.entries(filterMappings).map(
            ...
            <TableContainer>
                <Table css={tableStyle}>
                    ...
                    <TableHead>
                        <TableRow>
                            {showRank ? (
                                <TableCell>{rankLabel || "Rank"}</TableCell>
                            ) : (
                                <></>
                            )}
                            {Children.map(children, (child) => {
                                if (!child) return null;
                                const { label, align, sortable, sortProperty } =
                                    child.props;
                                if (sortable !== true)
                                    return (
                                        <TableCell
