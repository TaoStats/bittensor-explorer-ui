...
import { TransfersOrder, TransfersFilter } from "../../services";
import {
    ...
    const getExportCSV = async () => {
        ...
        if (!loading && !notFound && data !== undefined) {
            ...
            data.forEach((transfer: Transfer) => {
                const createdAt =
                    blockTimestamps[transfer.blockNumber.toString()];
        ...
        >
            ...
            {direction?.show && (
                <TransfersTableAttribute
                    ...
                    render={(transfer) => {
                        const dir =
                            transfer.from === direction?.source ? "out" : "in";
                        return (
                            <div css={dirContainer}>
                                <div css={dir === "out" ? dirOut : dirIn}>
                                    {dir}
                                </div>
                            </div>
                        );
            ...
            <TransfersTableAttribute
                ...
                render={(transfer) => (
                    <AccountAddress
                        ...
                        link={
                            direction?.show &&
                            transfer.from !== direction?.source
                                ? false
                                : true
                        }
                    />
