...
import { NeuronMetagraphOrder } from "../../services";
import { ItemsTable, ItemsTableAttribute, Link, Spinner } from "../elements";
...
export function NeuronMetagraphTable(props: NeuronMetagraphTableProps) {
                ...
                (
                    {
                        dailyReward,
                        stake,
                    }: { dailyReward: bigint; stake: bigint },
                    cur
                ) => {
                    return {
            ...
            <NeuronMetagraphTableAttribute
                ...
                render={({ validatorPermit, isImmunityPeriod }) => (
                    <div css={iconContainer}>
                        {validatorPermit && (
                            <img src={CheckShield} alt="validator" />
                        )}
                        {isImmunityPeriod && (
            ...
            <NeuronMetagraphTableAttribute
                ...
                render={({ emission, validatorTrust }) => (
                    <>
                        {emission >= 0 &&
                            formatNumber(
                                rawAmountToDecimalBy(validatorTrust, 65535),
                                {
                                    decimalPlaces: 5,
                                }
                            )}
            ...
            <NeuronMetagraphTableAttribute
                ...
                render={({ emission, consensus }) => (
                    <>
                        {emission >= 0 &&
                            formatNumber(
                                rawAmountToDecimalBy(consensus, 65535),
                                {
                                    decimalPlaces: 5,
                                }
                            )}
            ...
            <NeuronMetagraphTableAttribute
                ...
                render={({ emission, incentive }) => (
                    <>
                        {emission >= 0 &&
                            formatNumber(
                                rawAmountToDecimalBy(incentive, 65535),
                                {
                                    decimalPlaces: 5,
                                }
                            )}
            ...
            <NeuronMetagraphTableAttribute
                ...
                render={({ emission, dividends }) => (
                    <>
                        {emission >= 0 &&
                            formatNumber(
                                rawAmountToDecimalBy(dividends, 65535),
                                {
                                    decimalPlaces: 5,
                                }
                            )}
            ...
            <NeuronMetagraphTableAttribute
                ...
                render={({ emission }) => (
                    <>
                        {emission >= 0 &&
                            formatNumber(
                                rawAmountToDecimal(emission.toString()),
                                {
                                    decimalPlaces: 5,
                                }
                            )}
                ...
                render={({ stake }) =>
                    taoPrice.loading ? (
                        ...
                        <span css={whiteText}>
                            $
                            {formatNumber(
                                rawAmountToDecimal(stake.toString()).mul(
                                    taoPrice.data || 0
                                ),
                                {
                                    decimalPlaces: 0,
