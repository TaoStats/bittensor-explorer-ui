...
import { DelegatesOrder, DelegateFilter } from "../../services";
import {
    ...
    const getExportCSV = async () => {
        ...
        if (!loading && !notFound && data !== undefined) {
            ...
            data.forEach((delegate: Delegate) => {
                const createdAt =
                    blockTimestamps[delegate.blockNumber.toString()];
            ...
            <DelegatesTableAttribute
                ...
                render={(delegate) =>
                    delegate.extrinsicId && (
                        <Link
                            to={`/extrinsic/${delegate.blockNumber}-${zeroPad(
                                delegate.extrinsicId,
                                4
                            )}`}
                        >{`${delegate.blockNumber}-${zeroPad(
                            delegate.extrinsicId,
                            4
                        )}`}</Link>
                    )
            ...
            <DelegatesTableAttribute
                ...
                render={({ action }) => (
                    <div css={dirContainer}>
                        <div css={action === "UNDELEGATE" ? dirIn : dirOut}>
                            {action}
                        </div>
                    </div>
                )}
